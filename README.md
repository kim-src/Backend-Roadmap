...
> 그렇다면 벽면 또는 와이파이 라우터에 있는 랜포트의 종착 지점은 어디일까요?  
...
> 반복하자면 모뎀의 역할은 네트워크의 디지털 신호와 전화의 아날로그 신호를 교환해주는 것입니다.  
> 참고로 디지털(digital) 신호는 숫자로 표현된 정보, 아날로그(analog) 신호는 진동, 전압 등의 물리량을 뜻합니다.  
...
### 📌 Case 4. ISP와 모뎀의 연결
> 근래 네트워크는 위와 같이 노드(컴퓨터 등 IP 주소를 보유한 전자기기), 라우터, 모뎀이 연결된 상태입니다.
> 같은 말로 우리의 네트워크 디지털 정보는 데이터 케이블을 따라 라우터, 모뎀을 통과합니다.
> 이 과정에서 
> 메시지는 이더넷 케이블을 지나 라우터를 통해 먼저 ISP 를 거치고, ISP 네트워크를 통해 다른 네트워크로 전송된다.


ISP는 모뎀과 네트워크 정보를 담고 있는 라우터를 연결해주는 인터넷 서비스 제공업체입니다.
