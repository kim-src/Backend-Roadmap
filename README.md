...
> (노드(node) = 컴퓨터, 스마트폰 등의 IP 주소를 보유한 모든 전자기기)
> 
> <br/>
>
...
> <br/>
> 
> 그런데 라우터끼리만 연결된 네트워크의 규모는 LAN 규모의 네트워크보다 확장되기 어려웠을 것입니다.  
> 그렇다면 LAN 규모였던 네트워크를 WAN 규모로 확장시킬 수 있었던 방법은 무엇일까요?  
>
> <br/>
>
> 불가능할 것 같았던 거대 규모의 네트워크 확장은 전화 통신 인프라로 해결되었습니다.  
...
> 그리고 이러한 인터넷이 정상 작동하려면 노드(node), 라우터(router), 모뎀(modem)이 동작하여야 됩니다.  
> 참고로 노드는 컴퓨터 등의 IP 주소를 보유한 모든 전자기기를 뜻합니다.  
> 
> <br/>
> 
> 노드, 라우터, 모뎀이 연결된 모식도는 아래와 같습니다. (출처: Mozilla)  
...
> 반복된 표현으로, 우리의 네트워크 디지털 정보는 데이터 케이블을 따라 라우터 및 모뎀으로 전송됩니다.  
