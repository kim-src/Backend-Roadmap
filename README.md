...
> 그렇다면 벽면 또는 와이파이 공유기에 있는 랜포트의 종착 지점은 어디일까요?  
...
### 📌 Case 1. 노드끼리의 연결
> IP 주소를 보유한 서로 다른 노드끼리 연결시킬 수 있는 가장 간단한 방법은  
...
> <br/>
> 
> 하지만 수많은 노드가 연결된 네트워크 환경을 구축하기 위해서는  
...
> Case 1에서의 단점을 보완해주기 위해 개발된 것이 공유기(home router)입니다.  
...
> <br/>
>
> 이와는 달리 노드(컴퓨터 등 IP 주소를 보유한 전자기기)에서 발생된 네트워크 정보는 디지털(digital) 형태입니다.  
> 따라서 네트워크 망을 확장시키기 위해서는 디지털 신호와 아날로그 신호를 교환해주는 장치가 필요한 상황이었습니다.  
> 그래서 모뎀이 개발되었으며 모뎀의 역할은 디지털 및 아날로그 신호를 서로 변환해주는 것입니다.  
>
> <br/>
>
...
> 참고로 노드는 컴퓨터 등의 IP 주소를 보유한 모든 전자기기를 뜻합니다.  
> 
> <br/>
> 
...
### 📌 ISP의 네트워크 관리
> 근래 네트워크는 위와 같이 노드(컴퓨터 등 IP 주소를 보유한 전자기기), 공유기, 모뎀이 연결된 상태입니다.  
> 반복된 표현으로, 우리의 네트워크 디지털 정보는 데이터 케이블을 따라 공유기 및 모뎀으로 전송됩니다.  
> 데이터 송신 및 수신이 발생하는 이러한 네트워크 환경의 규모가 커지면 개인이 네트워크를 관리하기는 어렵습니다.
>
> <br/>
>
> 그래서 거대한 네트워크를 관리하는 업체가 존재하게 됐으며 해당 업체를 ISP (Internet Service Provider)라고 합니다.
이 ISP는 역할에 따라 접속 ISP, 호스팅 ISP, 통행 ISP 등으로 구분되며 이에 대해서는 추후 말씀드리겠습니다.
> 메시지는 이더넷 케이블을 지나 공유기를 통해 먼저 ISP 를 거치고, ISP 네트워크를 통해 다른 네트워크로 전송된다.


ISP는 모뎀과 네트워크 정보를 담고 있는 공유기를 연결해주는 인터넷 서비스 제공업체입니다.
