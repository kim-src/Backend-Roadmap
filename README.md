...
> <br/>
>
> 이와는 달리 노드(IP 주소를 보유한 전자기기)에서 발생된 네트워크 정보는 디지털(digital) 형태입니다.  
> 따라서 네트워크 망을 확장시키기 위해서는 디지털 및 아날로그 신호를 교환해주는 장치가 필요한 상황이었습니다.  
> 그래서 모뎀이 개발되었으며 모뎀의 역할은 디지털 신호와 아날로그 신호를 서로 변환해주는 것입니다.  
>
> <br/>
>
...
### 📌 ISP의 네트워크 관리
> 근래 네트워크는 상기 내용과 같이 노드(컴퓨터 등), 공유기, 모뎀이 연결된 상태입니다.  
> 반복된 표현으로, 우리의 네트워크 디지털 정보는 데이터 케이블을 따라 공유기 등으로 전송되며 공유됩니다.  
> 그런데 데이터 송수신이 발생되는 이러한 네트워크 환경의 규모가 커질수록 개인이 네트워크를 관리하기 어렵습니다.  
>
> <br/>
>
